...
of French and English text. We will notate that $|F_s| = M_s$ and $|E_s| = N_s$. The model selects for each target word $e_{s,n}$ an
...
We will assume that each $\phi_f$ is a $|V|_E$-dimensional Dirichlet with parameters ${\alpha_f}_e$, all of which are initially set to $\alpha_0$, a hyperparameter.
...
\node[draw=none] at (3.85, 2.45) {$|V|_F$};
